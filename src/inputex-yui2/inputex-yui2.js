...
//Gi.console = (console && console.log ) || (function(){});
console.log = console.log;
Gi.modulesKeys = [];
Gi.addModule = function(){
    var modules = Gi.modules,keys = Gi.modulesKeys, key, requires, fn;
    for(var i = 0; i < arguments.length; i++){
        if(typeof(arguments[i]) == "function" && typeof(key) === "string"){
            ...
            if(Gi.modulesKeys.indexOf(arguments[i]) !== -1){
                console.log(arguments[i]+" ever been loaded");
                return;
            } else {
                Gi.modulesKeys.push(arguments[i]);
                key = arguments[i];
...
Gi.indexOf = function(array,el){
    return array.indexOf(el);
}
Gi.use = function(){
    var args = [];
    var scope = new Gi.localInputEx();
    var loaded = [];
    var loadModule = function (moduleName){
        var moduleConf = Gi.modules[moduleName], bool;
        if(typeof(moduleConf)=== "undefined"){
            console.log(moduleName+" not loaded");
            return false;
        }
        // if not loaded
        if(Gi.indexOf(loaded, moduleName) === -1){
...
//  */
// 
// YUI.add("inputex-yui2", function(Y){
...
// });
// 
// YUI.add("inputex-yui2-lang", function(Y){
//     //lI means localInputEx
//    Y.lI = {};
//    Y.lI.Lang = Y.Lang;
//    Y.lI.UA = Y.UA;
